...
Current checkpoint: [Phase 2.7C](docs/phase-2-7c-approve-reject.md) is SAP runtime-verified complete. Root actions `submit`, `approve` and `reject` carry business `Status` from `DRAFT` to `SUBMITTED` and then to `APPROVED` or `REJECTED`, rejection requiring a reason and recording `RejectionOrigin = APPROVER`. RAP prechecks enforce the invariant that commercial content is editable only while `Status` is `DRAFT`, covering root updates, item updates, create-by-association and [Phase 2.6](docs/phase-2-6-technical-draft.md) `removeItem`, for active and technical draft instances. RAP technical draft identity remains distinct from business `Status = 'DRAFT'`, and authorization is still the permissive study stub. Still pending: `cancel`, root-deletion lifecycle rules, and `sendToSupplier`, which is deferred to Phase 5 with `DeliveryIntent`. `PurchaseOrderNumber` remains unresolved for Phase 2 closure: the target is to be investigated for a released number-range mechanism, implemented in Phase 2 if one is suitable, and otherwise made the subject of an explicit documented deferment decision without naming a destination phase in advance.
