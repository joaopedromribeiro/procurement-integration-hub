...
Current checkpoint: [Phase 2.7B](docs/phase-2-7b-post-submission-immutability.md) is SAP runtime-verified complete. Once business `Status` is `SUBMITTED`, RAP prechecks reject root commercial updates, item commercial updates and create-by-association, and a status guard rejects `removeItem`; a technical draft taken from a submitted order carries `SUBMITTED` and is rejected the same way. It builds on [Phase 2.7A](docs/phase-2-7a-submit-action.md), the active-only `submit` transition, and [Phase 2.6](docs/phase-2-6-technical-draft.md), where root-bound `removeItem` checks ownership, uses managed internal child DELETE and recalculates header totals. RAP technical draft identity remains distinct from business `Status = 'DRAFT'`. Still pending: `PurchaseOrderNumber` allocation, root-deletion lifecycle rules, and the approve, reject, sendToSupplier and cancel transitions.
