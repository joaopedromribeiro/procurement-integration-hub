...
Status: **SAP runtime-verified complete** for active, saved-draft and buffer-only draft item removal, total recalculation and ownership protection. Phase 2.7A `submit`, Phase 2.7B post-submission immutability, Phase 2.7C `approve`/`reject` and Phase 2.7D-1 `cancel` are runtime-verified complete on top of this baseline; the Phase 2.7D-2 root `DELETE` narrowing and Phase 2.7E `PurchaseOrderNumber` allocation have since completed Phase 2; `sendToSupplier` belongs to Phase 5.
