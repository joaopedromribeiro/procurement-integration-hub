...
**What this decision does not settle.** Choosing the path is not walking it. Nothing is deployed, nothing is bound, and no tunnel is configured, so **blocker B3 stays open** and there is still no reachable CAP HTTPS URL for SAP to call. The SAP-side half is equally unsettled: how an outbound HTTPS target is declared on this system — a communication scenario and arrangement, an `SM59`-style HTTP destination, or a URL destination — is **not established by this repository** and remains probes **P2** and **P3**, with certificate trust as **P4**. Naming the preferred path does not verify the mechanism that walks it; treat the SAP leg as unproven until ADT or the system returns an answer. Trial accounts also carry evaluation limits and lifecycle constraints that are environment-specific and must be read from the cockpit rather than from this document.
