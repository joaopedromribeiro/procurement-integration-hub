...
| A CAP URL the SAP system can actually resolve and reach | **A URL now exists; whether SAP can resolve and reach it is still unverified.** CAP is deployed to Cloud Foundry at `https://0badc38dtrial-dev-cap-supplier-portal-srv.cfapps.us10-003.hana.ondemand.com`, replacing the original `http://localhost:4004` that SAP could never have reached, and authenticated access is runtime-verified **from the developer machine**. **No request has come from the ABAP system**, so SAP-side DNS, network path and TLS trust for this host remain unproven. The endpoint requires an OAuth2 token carrying `IntegrationClient` | deployment side done; SAP side needs probes P1–P4, see B3 |
...
**What this decision does not settle.** Choosing the path is not walking it, and walking half of it is not finishing it. The chosen option has since been carried out — CAP is deployed to Cloud Foundry, bound to `pih-hdi` and a real XSUAA instance, running 1/1 behind a stable HTTPS route — so the half of **blocker B3** that said *no CAP URL exists* is resolved. **B3 itself stays open**, because nothing has yet been proven from the SAP side: S/4 has never called that route, so its DNS, network path and TLS trust for this host are unverified, and no outbound destination exists there. The SAP-side half is equally unsettled: how an outbound HTTPS target is declared on this system — a communication scenario and arrangement, an `SM59`-style HTTP destination, or a URL destination — is **not established by this repository** and remains probes **P2** and **P3**, with certificate trust as **P4**. Naming the preferred path does not verify the mechanism that walks it; treat the SAP leg as unproven until ADT or the system returns an answer. Trial accounts also carry evaluation limits and lifecycle constraints that are environment-specific and must be read from the cockpit rather than from this document.
...
| **B3** | No CAP URL is reachable from SAP. **PARTIALLY RESOLVED — the deployment side only. Still open for SAP reachability.** **Resolved:** the original problem was that no CAP URL existed at all, only `http://localhost:4004`. CAP is now deployed to Cloud Foundry with a stable public HTTPS route, `https://0badc38dtrial-dev-cap-supplier-portal-srv.cfapps.us10-003.hana.ondemand.com`, the application runs 1/1, the route is reachable **from the developer machine**, and XSUAA authentication against it is runtime-verified. **Still open:** every SAP-side leg. **SAP S/4 has never called this route**, so DNS and network reachability from the ABAP system to this exact host is unverified, TLS trust for this exact Cloud Foundry certificate chain is unverified (probe P4), and no outbound destination or communication configuration exists on the SAP side at all. Probes **P1–P4** remain incomplete. The route is also authenticated rather than open: SAP must present an OAuth2 client-credentials token carrying `IntegrationClient`. **B3 closes only when an outbound call from S/4 actually reaches the deployed CAP endpoint.** | The outbound leg now has a target; SAP reaching it is unproven | **The path is now chosen** — `S/4 sandbox → HTTPS → CAP deployed to BTP Cloud Foundry`, per the decision in 1.3, with a public tunnel kept only as a fallback. It is no longer entitlement-blocked: BTP Trial Cloud Foundry is available in `us10-003`, space `dev` is usable, `pih-hana` (`hana-cloud` / `hana-free`) and `pih-hdi` (`hana` / `hdi-shared`) exist, and the CAP production profile is prepared for HANA. **B3 itself remains open**, because the application is **not deployed and not bound** — there is still no reachable CAP HTTPS URL an SAP system could call — and the SAP-side declaration of that target is still probes P2/P3 with certificate trust as P4. Record the deployment in `docs/environments.md` once it exists |
