...
**P1, P2/P3, P4 and P12 are answered by the outbound runtime probe; P5–P11 remain open and are now the critical path.** The transport seam is no longer the blocker — the remaining unknowns are all about *building the payload* rather than sending it: UUID formatting (P5), decimal formatting under a comma-decimal user (P6), timestamp formatting (P7), the table's unique secondary index (P8), `abap.string` as a non-key column (P9), JSON serialization with exact camelCase (P10), and the actual persisted values of the integration fields (P11). **No ABAP HTTP call was authored from memory in this project, and none will be** — the chain above is recorded because it executed, not because it looked right.
...
*Since this section was written, the connectivity half of 5.1 has been carried out and runtime-verified — CAP was deployed to Cloud Foundry, SAP-side destinations and OAuth2 were configured, certificate trust was fixed, and an outbound probe reached the deployed endpoint (see 1.5). So the original sentence "no HTTP request was made in either direction" no longer holds, and **blocker B3 is resolved**. The design half of 5.1 is unchanged and still not runtime-verified: no `DeliveryIntent`, no builder, no mapper, no transport class and no `sendToSupplier`.*
