...
**Answered: P1, P2/P3, P4 and P12** by the outbound runtime probe, **P11** by the Phase 5.2a runtime evidence, **P8 and P9** by the Phase 5.2b activation, and **P5, P6 and P10** by the Phase 5.2d probes. **Open: P7, P13 and P14** — and **P15 is ANSWERED**, by the `ZJP_CAP_BASE` probe. **P14 does not block Phase 5.2e**: the transport generates no identity of any kind. It blocks the 5.2g coordinator, which must create a correlation ID it can persist. P14 and P15 were added while reviewing what Phase 5.2e needs, the same way P13 was added for 5.2b — the original list had no entry for either. Neither blocks Phase 5.2d: the outbound `Orders` body carries no timestamp, and nothing computes `payload_hash` until 5.2f. The transport seam is no longer the blocker and neither is persistence — every remaining unknown is about *building* the payload rather than sending or storing it.
...
**Correlation ID: unchanged from discovery, and the seam stays untouched.** The transport generates nothing. The 5.2g coordinator will create the correlation UUID, persist it to `ZJP_PO_DLV-last_correlation_id` and pass `X-Correlation-ID` as an ordinary header, which the adapter forwards without knowing what it is. **P14 stays OPEN** — nothing in this project has ever created a UUID — and it does **not** block 5.2e, only the coordinator.
...
**P7 and P13 stay open and neither blocks this slice.** The outbound body has no timestamp field, and nothing computes `payload_hash` until Phase 5.2f. **P14 stays open too**, which is why the harness used a fixed delivery identity.
...
| `sendToSupplier`, durable intent creation and the `payload_hash` — **next, not started**; needs **P13** and must check for and reuse an existing intent | 5.2f |
