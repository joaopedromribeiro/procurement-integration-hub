...
| 2.7 Business actions | 2.7A `submit`, 2.7B immutability and 2.7C approve/reject runtime-verified; cancel and sendToSupplier pending |
...
1. Preserve the Phase 2.1–2.7C runtime-verified baseline: the `removeItem` deletion boundary, the active-only `submit`, `approve` and `reject` transitions, and the precheck-enforced lifecycle invariant.
2. Implement `cancel` (Phase 2.7D) before declaring Phase 2 complete, together with the root `DELETE` narrowing that belongs with it.
...
4. Treat `PurchaseOrderNumber` as unresolved for Phase 2 closure rather than as settled work. Before Phase 2 is declared complete, investigate the target for an appropriate released number-range mechanism. If a suitable mechanism exists, implement it within Phase 2. If it cannot reasonably be implemented within the Phase 2 target and scope, make and document an explicit architectural deferment decision at that time, and do not name a destination phase before that decision. This is the OI-02 scope.
...
Phases 2.7A, 2.7B and 2.7C are all SAP runtime-verified. The commercial lifecycle is now closed at both ends: `submit`, `approve` and `reject` move business `Status` through `DRAFT` → `SUBMITTED` → `APPROVED`/`REJECTED`, and content is editable only while `Status` is `DRAFT`. `Status` `INITIAL` passes the prechecks only as a transient creation-compatibility case, never as a business state. The following gaps remain and must not be reported as solved. `cancel` is not implemented, so no order can be withdrawn. Root `DELETE` is still allowed at any status; narrowing it belongs with the `cancel` rules. `sendToSupplier` is deliberately deferred to Phase 5 because `DeliveryIntent` and dispatch semantics are introduced there, and a status-only action would name a capability that does not exist. `PurchaseOrderNumber` remains unresolved for Phase 2 closure and is still unallocated: OI-02 keeps it inside Phase 2 until the number-range investigation yields either an implementation or an explicit, documented deferment decision. Supplier-side rejection, which sets `RejectionOrigin = 'SUPPLIER'`, is separate future work. Authorization is still the permissive study stub: no approver-role enforcement exists, and the word "approve" names a transition, not an access control. `Activate` over a closed-status technical draft is untested, instance feature control is not implemented, and no multi-user or concurrency coverage is claimed — the saved-draft `LOCKED` observation is target-specific, single-user evidence, not a lifecycle guarantee.
