...
**Phase 8.3 RAP Buyer/Approver authorization is CLOSED on owner-supplied SAP ADT activation and UI OData V4 runtime evidence.** The Buyer can edit and perform Buyer actions but not approve/reject; the Approver can approve/reject but not edit or perform Buyer actions. Base behavior authorization also separates inbound Integration, Operator, and outbound Worker actions, without claiming runtime identity acceptance for those roles. Phase 8.1 and 8.2 remain closed; 8.4–8.6 have not started, and Phase 8 as a whole remains open. Phase 9 is Event Mesh. See [current Phase 8 status](PROJECT_STATUS.md#phase-8--current-security-checkpoint).
