...
**Phase 6 is complete and Phase 7 is active.** Phase 7 — error handling, monitoring and operational resilience — hardens the proven bidirectional flows in place and adds no new product; Event Mesh stays Phase 9 and API Management stays Phase 10. **7.2 and 7.3 are complete and runtime-verified. Phase 7.4a and 7.4b are implemented and locally verified but not deployed:** CAP now combines the frozen four-attempt durable retry policy with conservative `NOT_SENT` / `MAY_APPLY` transport certainty and bounded `Retry-After` parsing. Only proven DNS, refused-connect and explicit TLS certificate-verification failures become retryable; ambiguous no-answer results remain operator-only `UNKNOWN`. **7.4c–7.4e remain, so Phase 7.4 and Phase 7 as a whole are not complete.** See the [Phase 7 guide](docs/phase-7-operational-resilience.md).
