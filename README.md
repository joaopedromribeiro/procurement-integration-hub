...
**Phase 6 is complete and Phase 7 is active.** Phase 7 — error handling, monitoring and operational resilience — hardens the proven bidirectional flows in place and adds no new product; Event Mesh stays Phase 9 and API Management stays Phase 10. **7.2 and 7.3 are complete and runtime-verified. Phase 7.4a is implemented and locally verified but not deployed:** the CAP response parent now persists durable retry eligibility under a four-attempt budget, positive-only 0–20% jitter over 5 s / 30 s / 120 s bases, and an inclusive 15-minute retry window, while reporting before-due, retry-exhausted and retry-window-blocked rows separately. `UNKNOWN` is still operator-only, the attempt child is unchanged, and no scheduler exists. **7.4b–7.4e remain, so Phase 7.4 and Phase 7 as a whole are not complete.** See the [Phase 7 guide](docs/phase-7-operational-resilience.md).
