...
**Phase 4 has started: the CAP Supplier Portal foundation is locally runtime-verified.** `cap-supplier-portal` installs, typechecks, tests and starts on CAP 10 / Node.js 22 with TypeScript, serving one non-business health endpoint over SQLite in memory. It has no domain model, no business API and no contact with SAP; those are Phases 4.2 to 4.5. This is local Node.js execution evidence, a different and weaker claim than the SAP runtime evidence behind Phases 1 to 3. See the [Phase 4.1 guide](cap-supplier-portal/docs/phase-4-1-cap-foundation.md).
...
| SAP CAP, Node.js, TypeScript, CDS, SQLite | Running portal API and handler tests | Phase 4.1 foundation locally runtime-verified; portal API is phases 4.2–4.4 |
...
CAP CDS models describe Suppliers, Orders, and composed OrderItems. TypeScript handlers enforce ingestion, supplier decisions, and row-level supplier access. SQLite supports local development; portable CDS and queries prepare for a later HANA Cloud deployment. Integration and supplier-facing services have separate permissions. The project itself now exists and runs locally: [Phase 4.1](cap-supplier-portal/docs/phase-4-1-cap-foundation.md) delivered the CAP 10 / Node.js 22 / TypeScript foundation with a non-business health service, and the entities, the ingestion contract and the supplier decision API are built in Phases 4.2 to 4.4.
...
The CAP Supplier Portal now runs locally. `cd cap-supplier-portal && npm ci && npm run typecheck && npm test && npm start` starts CAP on `http://localhost:4004`, where `GET /health/ping` answers `{"status":"UP", ...}`. That is the whole of [Phase 4.1](cap-supplier-portal/docs/phase-4-1-cap-foundation.md): a verified foundation with no domain model and no business API, which arrive in Phases 4.2 to 4.4. Node.js 22 or later is required. The portal does not contact SAP in Phase 4.
