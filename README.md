...
**Phase 8.2 interactive supplier login is runtime accepted within a read-only trial scope; Phase 8 remains open.** A standalone XSUAA-bound approuter authenticated a named `sap.default` human user, forwarded that user's token to the unchanged CAP SupplierService, and returned only `RTTEST001` orders. The browser route did not proxy IntegrationService. No supplier decision was made; cross-supplier isolation still needs a second active supplier. The owner accepted an unresolved approuter availability advisory only for the short acceptance window and stopped the router afterward (requested stopped, 0/1 instances; termination was still in progress in the last snapshot). See [current status](PROJECT_STATUS.md#phase-8--current-security-checkpoint) and the [portal security notes](cap-supplier-portal/README.md#phase-82-interactive-runtime-acceptance).

**Phase 8.3 RAP Buyer/Approver authorization is CLOSED on owner-supplied SAP ADT activation and UI OData V4 runtime evidence.** The Buyer can edit and perform Buyer actions but not approve/reject; the Approver can approve/reject but not edit or perform Buyer actions. Base behavior authorization also separates inbound `INTEGRATOR`, Operator, and outbound Worker actions. Phase 8.1 and 8.2 remain closed. **Phase 8.4 is CLOSED:** the two Cloud Integration sender routes use distinct dedicated clients and roles, SAP's integration action was functionally verified with `ZJP_ROLE=INTEGRATOR`, and both final deployed iFlow exports are synchronized and verified. The shared SAP Dialog identity is not least privilege. 8.5–8.6 have not started, so Phase 8 remains open. Phase 9 is Event Mesh. See [current status](PROJECT_STATUS.md#phase-8--current-security-checkpoint) and the [8.4 record](docs/phase-8-4-technical-boundaries.md).
