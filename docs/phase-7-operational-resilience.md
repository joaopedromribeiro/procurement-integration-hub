...
**Phase 7 CLOSED within recorded acceptance scope, 2026-09-23:** 7.5 concurrent-drainer and stale-lease reclaim proofs passed in the deployed CAP runtime; 7.6 recovery mechanism passed but ended in answered HTTP 400 `UNKNOWN_SUPPLIER` / FAILED, not DELIVERED; 7.7 SAP and CAP status reporters ran; and the [Phase 7.8 runbook](phase-7-operational-runbook.md) records the outcomes. ACTIVE SAP source and both final deployed iFlow exports are synchronized; the SupplierResponse final export passed verification with its normal CSRF target and no OQ-2 override. The complete unchanged CAP suite passed outside the Codex host: 231/231 tests in 44 suites, zero failures; typecheck, production CDS build and `git diff --check` passed. The earlier Windows `tsx` startup failure was host-only.
...
**Subphase 7.3 is COMPLETE and runtime-verified**, executed 2026-09-21 on a fresh SAP order `PO00000160`. It is the first Phase 7 subphase to change executable source: a `SupplierResponseDeliveryAttempts` child on the CAP inbound leg and a single `attempt_count` column on the SAP delivery intent. Subsequent deployed Phase 7 results and limits are recorded in the final evidence section below; older phase-checkpoint text is retained as historical evidence, not current state.
...
| 7.4 | Retry / transient failure policy | **Implemented and deployed; ACTIVE SAP source and final restored SupplierResponse export synchronized; OQ-2 exercised** |
| 7.5 | Inbound concurrency hardening | **deployed race and stale-lease reclaim PASS** |
| 7.6 | Outbound recovery parity / `retryDelivery` | **mechanism PASS; answered 400 persisted FAILED; UNKNOWN → DELIVERED not demonstrated** |
| 7.7 | Monitoring and correlation | **SAP/CAP reporters runtime-proven; SAP correlation matched CAP header** |
| 7.8 | Operational runbook | **complete, with fail-closed multi-UNKNOWN guidance** |
| 7.9 | Runtime resilience tests and closure | **CLOSED within documented evidence limits; full 231-test suite and final exports verified** |
...
### What Phase 7.3 alone did **not** prove (historical checkpoint)
...
**B. Not a real concurrency race at the 7.3 checkpoint.** No simultaneous HANA race was manufactured then; the automated test proved the narrower invariant — a losing guarded compare-and-set writes **zero** child rows — against the in-memory database. Phase 7.5 subsequently supplied the deployed two-drainer proof: one claim owner, one competing skip, and exactly one transport attempt/history entry.

**C. `retryDelivery` was not yet implemented at the 7.3 checkpoint.** `ZJP_CL_DISPATCH_RETRY_RUNNER` is **not** the production capability: it is a one-off, fixture-specific runtime harness and must not be promoted. Phase 7.6 subsequently built the EML-only `retryDelivery` action and exercised its recovery mechanism on the existing intent under the same `DeliveryUUID`; the answered HTTP 400 ended FAILED, so UNKNOWN → DELIVERED remains unproven.
...
| 7.5 | **PASS:** concurrent deployed flushes had one owner, one skip, one attempt/history; an abandoned expired lease was reclaimed, attempted once and cleared | Deployed evidence |
| 7.6 | **PARTIAL:** same existing UNKNOWN intent and snapshot reused, one preparation and one dispatch; answered HTTP 400 `UNKNOWN_SUPPLIER` persisted FAILED at attempts 2. UNKNOWN → DELIVERED was not demonstrated because no remaining UNKNOWN snapshot has an active CAP supplier | Supplier-data limitation, not a production-code defect |
| 7.7 | **PASS:** both deployed reporters returned real census; SAP `LastCorrelationId` matched CAP request `X-Correlation-ID`. Indexed MPL search must have its own monitor evidence | Deployed evidence |
| 7.8 | Runbook complete and updated for production runner's exact-one-UNKNOWN fail-closed gate | 7.2–7.7 |
| 7.9 | Runtime evidence recorded; final restored SupplierResponse export verified; complete unchanged suite passed 231/231 in 44 suites outside the Codex host; typecheck, production build and `git diff --check` passed | CLOSED within documented evidence limits |

## Final Phase 7.5–7.9 runtime and repository evidence (2026-09-22–23)

- **7.5 PASS:** two concurrent deployed CAP drainers produced one claim owner, one competing skip, and exactly one HTTP attempt/history row. A separate deliberately abandoned `IN_FLIGHT` lease expired; the next runner reclaimed the same row, recorded exactly one real attempt and cleared the lease.
- **7.6 recovery mechanism PASS, delivery success not shown:** the existing UNKNOWN `DeliveryUUID 37FC3FA8EB2D1FE1ADD3A6790C9BD871` for `PurchaseOrderUUID 37FC3FA8EB2D1FE1ADD3A6790C9A3871` was reused with its durable snapshot and no new intent. One `retryDelivery` preparation and one coordinator dispatch changed attempts 1 → 2. The result was `answered=true`, HTTP 400 `UNKNOWN_SUPPLIER`, correctly persisted FAILED. Correlation `37FC3FA8EB2D1FD1ADDDABB98EC8F0E8` was the same SAP `LastCorrelationId` and CAP request `X-Correlation-ID`. Every remaining UNKNOWN snapshot names SUP044, SUP074A or SUP074H, none active in CAP; no supplier master data or durable snapshot was changed to manufacture UNKNOWN → DELIVERED. The tested FAILED intent must not be retried.
- **7.7 PASS:** the SAP read-only `ZJP_CL_DELIVERY_STATUS` and CAP `operational-status:prod` ran against deployed state. Final SAP census: PENDING=85, IN_FLIGHT=5, DELIVERED=24, FAILED=32, UNKNOWN=21, live leases=0, stale leases=5. Historical SAP stale leases remain out of scope. Final CAP census: DELIVERED=5, UNKNOWN=1, PENDING=3, retryWindowBlocked=3, live leases=0, stale leases=0, historyCount=5. The historical Phase 7.2 UNKNOWN fixture remains untouched.
- **7.8:** the [runbook](phase-7-operational-runbook.md) is complete. The production recovery runner requires exactly one UNKNOWN globally and fails closed with the current 21; no generic recovery run is safe as-is.
- **7.9 final synchronization and validation:** ACTIVE ADT source mirrors were manually synchronized; `ZJP_I_PURCHASEORDER` and `ZJP_CL_PO_DISPATCH_TEST` were compared and required no sync. Both final deployed iFlow exports are synchronized. OrderDelivery retains the accepted configuration; SupplierResponse final export verification passed: normal CSRF service root, stage-scoped exception handling, accepted timeouts/correlation and no temporary OQ-2 override. The complete unchanged CAP suite passed outside the Codex host: 231 tests, 44 suites, 231 passed, 0 failed/cancelled/skipped/todo, duration 6865.0322 ms. `npm run typecheck`, `npx cds build --production` and `git diff --check` passed. The Codex Windows host's `uv_os_get_passwd` ENOMEM prevented test startup there only. No tests were weakened, no production behavior was changed, and no deployment or runtime acceptance command was executed during repository closure.

These findings do not authorize changes to `INTENT_VANISHED`, malformed successful-2xx receipt classification, SAP `I_TIMEOUT` (unit uncertain), the non-production `ZJP_CL_DISPATCH_RETRY_RUNNER`, production UI packaging, the CAP `reject()` warning, historical SAP stale leases or the Phase 7.2 UNKNOWN fixture.
...
**OQ-2 — resolved in the deployed flow and final export.** The inbound Exception Subprocess maps only the pre-CSRF `CSRF_FETCH` stage to answered HTTP 503 `UPSTREAM_CSRF_UNAVAILABLE` with `retryable=true`; later failures remain ambiguous and are not flattened. The controlled OQ-2 runtime test was completed, the tenant flow restored, and the final deployed SupplierResponse export verified with its normal CSRF service root and no temporary fault injection.
