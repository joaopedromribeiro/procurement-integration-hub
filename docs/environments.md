...
| Integration Suite capabilities and roles | **Resolved in Phase 6.1, in a dedicated subaccount.** The original trial subaccount had an unusable Cloud Integration runtime, refusing deployment with *“Requested runtime location (: cloudintegration) is not supported for tenant”*, so a dedicated **`PIH Integration`** subaccount was created with a clean Integration Suite subscription. Capabilities and roles actually used: `Integration_Provisioner`, **Build Integration Scenarios** and `PI_Integration_Developer`, plus a **SAP Process Integration Runtime** instance (service `it-rt`, plan `integration-flow`, role `ESBMessaging.send`, client-credentials grant) for inbound authorization. No secret material is recorded anywhere in this repository. Runtime location and region are tenant-specific and must be read from the cockpit rather than generalized from this record |
