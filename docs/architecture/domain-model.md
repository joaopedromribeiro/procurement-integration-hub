...
erDiagram
    ...
    PurchaseOrder ||--o{ DeliveryIntent : dispatches
    SupplierResponseDelivery ||--o{ SupplierResponseDeliveryAttempt : records
...
| SupplierResponseDelivery, phase 5 | responseId: UUID; order association; version; immutable response payload; state PENDING/DELIVERED/FAILED/UNKNOWN; attempts; lastAttemptAt; lastError; lastCorrelationId; timestamps |
| SupplierResponseDeliveryAttempt, phase 7.3 | ID: UUID; delivery: parent association; attemptNumber: Integer; correlationId: UUID nullable; startedAt: Timestamp; durationMs: Integer; outcome: PENDING/DELIVERED/FAILED/UNKNOWN; httpStatus: Integer nullable; errorCategory: NONE/REFUSED/TRANSIENT/AMBIGUOUS/NO_ANSWER/PAYLOAD; errorSummary: String(255) nullable. **Inbound CAP → SAP transport attempts only.** Diagnostics and audit: nothing reads it to decide anything, and the parent's four fields remain the durable fast path |
...
`DeliveryIntent` in RAP, introduced in Phase 5, stores delivery ID, root UUID, revision, immutable snapshot/hash, dispatch state, correlation, lease and original approval evidence. It also stores the **receipt's `portalOrderId`**, which the Phase 4.6 handoff assigns to SAP — *"what SAP stores to refer to the portal's copy"* — and for which no column exists on `PurchaseOrder` in this document or in `ZJP_PO_H`. It belongs here rather than on the header because a receipt is the result of one delivery, not a property of the order: one row per delivery keeps each receipt beside the payload that earned it, where a header field would be overwritten by a later revision. **Phase 7.3 extended the outbound side by exactly one field and no more: `attempt_count`, surfaced as `AttemptCount`, counting outbound SAP → Cloud Integration → CAP transport attempts.** There is **no SAP DeliveryAttempt table, child entity or composition**, and the generic shape this document once anticipated for it — a direction discriminator, an end timestamp and a next-retry time — was deliberately **not** built: direction can hold only one value in a table that is one-directional by construction, and an end timestamp duplicates `startedAt + durationMs`. Durable next-retry timing now exists only on the CAP inbound parent as Phase 7.4a's nullable `nextAttemptAt` and `retryWindowStartedAt`; SAP parity remains Phase 7.4c. The durable inbound history lives on the CAP side as `SupplierResponseDeliveryAttempts`, tabulated above. A fuller outbound history is reconsidered only if Phase 7.9 shows an outbound diagnosis genuinely unreachable from `attempt_count` and the existing last-attempt fields. These operational records are not editable commercial items and should be exposed read-only where needed. The minimum field set, the reason it is a separate managed RAP business object rather than a composition child or a plain table, and which of the two contract DTOs the snapshot freezes are designed in the [Phase 5.1 outbound foundation](../../abap-rap/docs/phase-5-1-outbound-foundation.md); nothing is implemented.
