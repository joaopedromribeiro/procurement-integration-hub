...
The project exists as of Phase 4.1 and runs locally: CAP 10 on Node.js 22, TypeScript, SQLite in memory, with the resolved versions pinned by a committed lockfile. It currently serves one non-business health function and holds no entity, which is the deliberate Phase 4.1 boundary rather than an omission. See the [Phase 4.1 guide](cap-supplier-portal/docs/phase-4-1-cap-foundation.md) for the full subphase plan and the execution evidence.
